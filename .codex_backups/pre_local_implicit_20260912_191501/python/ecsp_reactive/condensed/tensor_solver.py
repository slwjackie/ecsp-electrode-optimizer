...
def run_tensor_solvers(solvers,output_dirs,execution_config,*,full_bc_config=None):
    ...
    retry=np.zeros(B,np.int64);caps=np.full(B,math.inf);counts=np.zeros(B,np.int64)
    ...
    while np.any(active):
        ...
        first=k.tensor(retry>=2,dtype=torch.bool)
        ...
        packed=torch.cat((dt[:,None],valid[:,None].double(),rec,diag.reshape(B,-1),velocity[:,None]),-1).cpu().numpy()
        accepted=packed[:,1].astype(bool)&active;dt_cpu=packed[:,0];vel_cpu=packed[:,-1]
        failed=active&~accepted
        for i in np.where(failed)[0]:
            solvers[i].rejected_steps+=1;retry[i]+=1
            caps[i]=dt_cpu[i]*.5
            if (retry[i]>s.max_retries or not math.isfinite(caps[i]) or caps[i]<s.min_dt):
                g = packed[i,2+len(RECORD_NAMES):-1].reshape(3,6)
                cap_max = float(g[:,4].max())
                limiter_max = float(g[:,5].max())
                budget_pass = bool(budget_ok[i].item())
                errors[i]=PropagationCandidateNumericalError(
                    f"Reactive tensor failure: "
                    f"time={ts[i]:.12g}s "
                    f"dt_attempt={dt_cpu[i]:.12g}s "
                    f"retry={retry[i]} "
                    f"chemistry_subcycles={int(subcycles_cpu[i])} "
        ...
        for i in np.where(accepted)[0]:
            ...
            g=packed[i,2+len(RECORD_NAMES):-1].reshape(3,6)
            v.step_number+=1;v.first_order_steps+=int(retry[i]>=2)
            ...
            v.exact_skipped_steps+=int(np.all(g[:,3]>0));v.max_rate_cap=max(v.max_rate_cap,float(g[:,4].max()))
            ...
            counts[i]+=1;retry[i]=0;caps[i]=math.inf
    ...
    for i,v in enumerate(solvers):
        ...
        if errors[i] is not None:
            ...
            _write_json(outs[i]/'PROPAGATION_FAILED.json',{'status':'failed','message':str(errors[i]),'time_after_onset_s':float(ts[i])})
