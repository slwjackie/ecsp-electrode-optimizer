...
No .item(), .numpy(), or host decisions on evolving fields occur in this module.
The caller transfers only a small per-case acceptance/report table each trial.
"""
from __future__ import annotations
import math
import numpy as np
import torch
from .chemistry import RHO, MX, MY, ENERGY, A1, A2, CATION, ANION, WATER, PVA, PRODUCT_WATER, EC_LP, NCONS
...
class TensorCondensedKernel:
    ...
    def temperature(self,rho,e):
        g,v,H,m=self.heat_grid,self.heat_values,self.heat_integrals,self.heat_slopes
        h=e-self.cold(rho)+self.heat_offset
    ...
    def rates(self,U,T):
        values=[]
        for i,(a,e,l) in enumerate(self.channels):
            alpha=U[...,4+i]/U[...,0]
            rate=torch.exp(torch.clamp(interp(alpha,a,l)-interp(alpha,a,e)/(self.R*torch.clamp(T,min=1.)),min=-100.,max=60.))
            values.append(torch.where(alpha<1,rate,torch.zeros_like(rate)))
    ...
    def trial(self,U,dt,first_order):
        self.trial_max_subcycles=torch.zeros(self.B,device=U.device,dtype=torch.int64)
        d=dt[:,None,None,None]
