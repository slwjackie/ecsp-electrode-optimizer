...
def geometry_contract(config, physics_grid_size: int) -> dict:
    ...
    return dict(schema=BOOTSTRAP_SCHEMA,
                ...
                novelty=dict(
                    cross_topology_max_iou=float(opt.get("bootstrap_cross_topology_max_iou",0.75)),
                    within_topology_max_iou=float(opt.get("bootstrap_within_topology_max_iou",0.92)),
                    cross_topology_reference_count=int(opt.get("bootstrap_cross_topology_reference_count",5)),
                ))
...
def _build_bootstrap_legacy(config, *, physics_grid_size:int, audit_dir:Path|None=None,
    ...
    accepted_masks_by_topology=[]
    novelty=contract.get("novelty",{})
    cross_iou=float(novelty.get("cross_topology_max_iou",0.75))
    within_iou=float(novelty.get("within_topology_max_iou",0.92))
    cross_refs=max(1,int(novelty.get("cross_topology_reference_count",5)))
    cached=None
    ...
    for ti,template in enumerate(templates):
        accepted=0;attempt=0;rejected=Counter();local_rows=[];local_masks=[]
        ...
        while accepted<nv and attempt<maximum:
            ...
            if dh in design_seen or ph in physics_seen:
                ...
                continue
            if local_masks:
                max_iou=max(mask_pair_iou(r.anode_mask,r.cathode_mask,a,c) for a,c in local_masks)
                if max_iou > within_iou + 1e-12:
                    rejected["within_topology_near_duplicate"] += 1
                    if cached is not None:raise BootstrapGeometryError("Cached bootstrap violates within-topology IoU novelty gate")
                    continue
            cross_bank=[pair for previous in accepted_masks_by_topology for pair in previous[:cross_refs]]
            if cross_bank:
                max_iou=max(mask_pair_iou(r.anode_mask,r.cathode_mask,a,c) for a,c in cross_bank)
                if max_iou > cross_iou + 1e-12:
                    rejected["cross_topology_near_duplicate"] += 1
                    if cached is not None:raise BootstrapGeometryError("Cached bootstrap violates cross-topology IoU novelty gate")
                    continue
            g["geometry_id"]=f"G000_{template['topology_id']}_V{accepted:03d}"
            population.append((g,r));design_seen.add(dh);physics_seen.add(ph);local_masks.append((r.anode_mask.copy(),r.cathode_mask.copy()))
            row=_row(g,r,physical);rows.append(row);local_rows.append(row)
            accepted+=1
        accepted_masks_by_topology.append(local_masks)
...
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
# ADAPTIVE_SYMMETRIC_BOOTSTRAP_V2
#
# G0 family policy:
#   1) Probe 1A1C for a bounded number of attempts.
#   2) Continue 1A1C only when its measured acceptance rate predicts that the
#      full quota can be filled efficiently.
#   3) Otherwise discard the probe candidates and restart the family as 2A2C.
#   4) One family therefore contains one component pair only.
# ---------------------------------------------------------------------------
def _adaptive_pair_iou(a1, c1, a2, c2):
...
def _adaptive_family_template(family, topology_index, pair):
    from .geometry_families import build_family_component
    from .geometry import HIDDEN_BUS_ASSUMPTION
    ...
    return {
        ...
        "topology_id":f"T{topology_index:02d}_{family}",
    }
...
def build_bootstrap(
    ...
    *,
    ...
    for ti,family in enumerate(TOPOLOGY_FAMILIES[:nt]):
        ...
        # ---------------------------------------------------------------
        # Commit ONLY the selected pair to G0.
        # ---------------------------------------------------------------
        for accepted_index,item in enumerate(selected_stage):

            g,r,physical,dh,ph,_mask=item

            g["topology_id"]=f"T{ti:02d}_{family}"
            g["component_pair_label"]=selected_label
            g["component_count_pair"]=[
                1,1
            ] if selected_label=="1A1C" else [2,2]

            g["geometry_id"]=(
                f"G000_T{ti:02d}_{family}_"
                f"{selected_label}_V{accepted_index:03d}"
            )

            population.append((g,r))
            design_seen.add(dh)
            physics_seen.add(ph)
            rows.append(_row(g,r,physical))

        cross_bank.extend(
            selected_masks[:cross_refs]
        )
        ...
        summary=dict(
            ...
            pair_usage={selected_label:nv},
            probe_1A1C=dict(
                attempts=min(attempts1,probe_attempts),
                accepted=probe_accepts,
                projected_additional_attempts=
                    projected_additional,
                promising=bool(promising),
        ...
        report["topologies"].append(summary)
        report["accepted_count"]=len(population)
        report["elapsed_s"]=time.perf_counter()-started
        ...
        if verbose:
            ...
            projected_text=(
                "NA"
                if projected_additional is None
                else str(projected_additional)
            )

            print(
                f"[geometry-bootstrap] "
                f"T{ti:02d}_{family}: "
                f"{nv}/{nv} unique feasible; "
                f"selected={selected_label}; "
                f"1A1C_probe={probe_accepts}/"
                f"{min(attempts1,probe_attempts)}; "
                f"projected_additional={projected_text}; "
                f"attempts={attempts_total}",
                flush=True,
            )

    report.update(
        status="passed",
        all_candidates_feasible_before_physics=True,
        design_unique_count=len(design_seen),
        physics_unique_count=len(physics_seen),
        total_attempts=sum(
            t["attempts"]
            for t in report["topologies"]
        ),
        elapsed_s=time.perf_counter()-started,
        maximum_design_area_relative_error=max(
            r["design_maximum_area_relative_error"]
            for r in rows
        ),
        maximum_physics_area_relative_error=max(
            max(
                r["physics_anode_area_relative_error"],
                r["physics_cathode_area_relative_error"],
            )
            for r in rows
        ),
        minimum_design_gap_mm=min(
            r["design_gap_mm"]
            for r in rows
        ),
        minimum_physics_gap_mm=min(
            r["physics_gap_mm"]
            for r in rows
        ),
        maximum_used_width_mm=max(
            r["maximum_effective_width_mm"]
            for r in rows
        ),
    )
