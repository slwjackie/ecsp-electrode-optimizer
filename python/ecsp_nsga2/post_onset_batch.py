...
from .propagation import (PropagationCandidateError,PropagationConfigurationError,
                          run_condensed_propagation,candidate_failure_payload)
...
def _cpu_init():
    ...
    try:
        ...
        # Some Apple BLAS builds expose no version string to threadpoolctl;
        # its optional post-import scan then raises AttributeError.  The
        # worker is already bounded by the three environment variables and
        # torch.set_num_threads(1), so retain that safe fallback instead of
        # terminating the spawned process pool.
        pass
            ...
            for i in successful:
                ...
                for name,value in [('reactive_euler',reactive[i])]+([('condensed_propagation',baseline_results[i])] if cfg['compare_backends'] else []):
                    if isinstance(value,PropagationCandidateError):
                        errors[name]=value;paired[name]=candidate_failure_payload(value)
