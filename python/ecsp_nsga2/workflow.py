...
from .geometry_open_grammar import (
    SYMMETRIC_COMPONENT_PAIRS,
    graph_distance,
    graph_feature_vector,
    make_open_grammar_template,
    mask_pair_iou,
)
from .nsga2 import (
    Individual,
...
class NSGA2ElectricalSolidWorkflow:
    ...
    def _initial_population(self) -> tuple[list[Individual], dict[str, RasterizedGeometry]]:
        ...
        # One reference per original G0 family for open-grammar novelty.
        self._grammar_reference_masks = []
        self._grammar_reference_features = []
        self._grammar_reference_signatures = set()
        _seen_reference_families = set()
        for _g, _r in entries:
            _family = str(_g.get("family", _g.get("topology_id", "")))
            if _family in _seen_reference_families:
                continue
            _seen_reference_families.add(_family)
            self._grammar_reference_masks.append((
                np.asarray(_r.anode_mask, dtype=bool).copy(),
                np.asarray(_r.cathode_mask, dtype=bool).copy(),
            ))
            self._grammar_reference_features.append(graph_feature_vector(_g))
            self._grammar_reference_signatures.add(topology_signature(_g))
    ...
    def _make_child(
        ...
        base_attempts = int(self.opt.get("offspring_generation_attempts", 40))
        max_attempts = base_attempts * 2 if mode == "new_grammar" else base_attempts
        for attempt in range(max_attempts):
            if mode == "crossover":
                ...
                pair = (1, 1) if attempt < base_attempts else (2, 2)
                open_index = 100000 + generation * 10000 + child_index * 100 + attempt
                template = make_open_grammar_template(
                    seed=self.seed + generation * 1000003 + child_index * 97 + attempt,
                    topology_index=open_index, pair=pair, limits=self.limits,
                )
                genome = instantiate_variant(
                    template, variant_index=attempt,
            ...
            if (na, nc) not in SYMMETRIC_COMPONENT_PAIRS:
                # Reject asymmetric child; do not repair topology after evolution.
                continue
            ...
            if mode == "new_grammar" and raster.constraint_violation <= 1e-12:
                signature = topology_signature(genome)
                features = graph_feature_vector(genome)
                max_iou_allowed = float(self.opt.get("new_grammar_max_iou_to_seed", 0.72))
                min_graph_distance = float(self.opt.get("new_grammar_min_graph_distance", 0.12))

                if signature in self._grammar_reference_signatures:
                    continue
                if self._grammar_reference_features:
                    nearest_graph = min(
                        graph_distance(features, ref)
                        for ref in self._grammar_reference_features
                    )
                    if nearest_graph < min_graph_distance:
                        continue
                if self._grammar_reference_masks:
                    maximum_iou = max(
                        mask_pair_iou(
                            raster.anode_mask, raster.cathode_mask, ref_a, ref_c
                        )
                        for ref_a, ref_c in self._grammar_reference_masks
                    )
                    if maximum_iou > max_iou_allowed:
                        continue
            ...
            if raster.constraint_violation <= 1e-12:
                ...
                ind.metrics["geometry_descriptors"] = raster.descriptors
                if mode == "new_grammar":
                    # Register this accepted open grammar so later immigrants
                    # must differ from prior immigrants too.
                    self._grammar_reference_masks.append((
                        np.asarray(raster.anode_mask, dtype=bool).copy(),
                        np.asarray(raster.cathode_mask, dtype=bool).copy(),
                    ))
                    self._grammar_reference_features.append(graph_feature_vector(genome))
                    self._grammar_reference_signatures.add(topology_signature(genome))
                return ind, raster
        if best is None:
            raise RuntimeError(
                f"{mode}: no feasible symmetric 1A1C/2A2C child after {max_attempts} attempts"
            )
    ...
    def _finalise_with_propagation(
        ...
        def refine_one(
            ...
            if metrics.get("postOnsetModelValid") is False:
                clone.constraint_violation += float(
                    metrics["postOnsetValidityConstraintViolation"]
                )
            elif not bool(metrics.get("propagationSucceeded", False)):
        ...
        # Keep every attempted candidate for audit, including an all-invalid
        # selection. Never send failed/model-invalid candidates to Pareto or
        # knee selection, independently of their objective/penalty values.
        valid_refined = [
            ind for ind in refined
            if bool(ind.metrics.get("propagationSucceeded", False))
            and bool(ind.metrics.get("postOnsetModelValid", True))
            and ind.metrics.get("postOnsetValidityConstraintViolation", 0.0) == 0.0
        ]
        refined_front = pareto_front(valid_refined)
        self._write_population_csv(
            refined,
        ...
        if not refined_front:
            strict_json_dump(
                final_dir / "NO_SUCCESSFUL_PROPAGATION_REFINEMENT.json",
                {
                    ...
                    "selected_count": len(candidates),
                    "model_validity_rejection_count": sum(
                        ind.metrics.get("postOnsetModelValid") is False
                        for ind in refined
                    ),
                    "message": "No valid successful post-onset candidate is eligible; no final recommendation was fabricated.",
                },
            )
            raise RuntimeError(
