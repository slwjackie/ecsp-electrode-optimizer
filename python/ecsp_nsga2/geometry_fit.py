...
def fit_area_components(components, limits, *, placement_boxes=None, obstacles=None):
    ...
    length_hi = 3.0
    length_lo = 0.55
    # Family-specific lower bound. For TREE/H_TREE this is 0.90;
    # all other families retain the existing fitter behaviour.
    length_lo = max(
        float(length_lo),
        float(getattr(limits, "minimum_fit_length_scale", length_lo)),
    )
    for comp, box in zip(components,boxes):
