#!/usr/bin/env python3
"""Audit the v8.2.1 tree against the immutable v8.2.0 input release."""
...
def _baseline_zip_files(
    ...
    if _sha256_file(archive_path) != expected_archive_sha256:
        raise ValueError("Provided ZIP SHA-256 does not match the v8.2.0 baseline")
        ...
        for row in report["modified"]:
            ...
            diff = "".join(
                difflib.unified_diff(
                    before_lines,
                    after_lines,
                    fromfile=f"v8.2.0/{path}",
                    tofile=f"v8.2.1/{path}",
                )
            )
...
def main() -> int:
    ...
    parser.add_argument(
        "--baseline-manifest",
        type=Path,
        default=root / "docs" / "V820_BASELINE_MANIFEST.json",
    )
    parser.add_argument(
        "--expected-changes",
        type=Path,
        default=root / "docs" / "V821_EXPECTED_CHANGES.json",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=root / "runs" / "v8_2_1_cpu_validation" / "preservation",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "manifest modifications and the v8.2.1 authoritative manifest addition"
        ),
    )
