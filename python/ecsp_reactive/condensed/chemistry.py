...
class BCTwoChannelChemistry:
    ...
    def advance_local(self, U, thermo, dt, *, concentration_floor,
        ...
        if (not np.isfinite(after).all() or not np.isfinite(final_temperature).all()
            ...
            def diagnostic_minimum(values):
                values = np.asarray(values)
                comparable = values[~np.isnan(values)]
                return (float(np.min(comparable)) if comparable.size
                        else float("nan"))

            def diagnostic_maximum(values):
                values = np.asarray(values)
                comparable = values[~np.isnan(values)]
                return (float(np.max(comparable)) if comparable.size
                        else float("nan"))

            alpha_allowance = np.maximum(alpha_roundoff, 1.0e-12)
            trigger_masks = (
                ("nonfinite_state", np.any(~np.isfinite(after), axis=-1)),
                ("nonfinite_temperature", ~np.isfinite(final_temperature)),
                ("alpha1_below_lower_tolerance",
                 alpha_after[:, 0] < -alpha_allowance[:, 0]),
                ("alpha1_above_upper_tolerance",
                 alpha_after[:, 0] > 1.0+alpha_allowance[:, 0]),
                ("alpha2_below_lower_tolerance",
                 alpha_after[:, 1] < -alpha_allowance[:, 1]),
                ("alpha2_above_upper_tolerance",
                 alpha_after[:, 1] > 1.0+alpha_allowance[:, 1]),
                ("cation_below_negative_tolerance",
                 inventory_after[:, 0] < -inventory_output_roundoff[:, 0]),
                ("anion_below_negative_tolerance",
                 inventory_after[:, 1] < -inventory_output_roundoff[:, 1]),
                ("pva_below_negative_tolerance",
                 inventory_after[:, 2] < -inventory_output_roundoff[:, 2]),
                ("temperature_below_tmin_tolerance",
                 final_temperature
                 < thermo.tmin-temperature_output_roundoff),
                ("temperature_above_tmax_tolerance",
                 final_temperature
                 > thermo.tmax+temperature_output_roundoff),
            )
            offending_indices = {
                name: int(np.flatnonzero(mask)[0])
                for name, mask in trigger_masks if np.any(mask)
            }
            alpha_bound_excess = np.maximum(
                np.maximum(-alpha_after, alpha_after-1.0), 0.0
            )
            negative_inventory_violation = np.maximum(
                -inventory_after, 0.0
            )
            endpoint_diagnostic = (
                f"nonfinite_state_count="
                f"{int(np.count_nonzero(~np.isfinite(after)))}, "
                f"nonfinite_state_cell_count="
                f"{int(np.count_nonzero(np.any(~np.isfinite(after), axis=-1)))}, "
                f"nonfinite_temperature_count="
                f"{int(np.count_nonzero(~np.isfinite(final_temperature)))}, "
                f"alpha1_min={diagnostic_minimum(alpha_after[:, 0])!r}, "
                f"alpha1_max={diagnostic_maximum(alpha_after[:, 0])!r}, "
                f"alpha2_min={diagnostic_minimum(alpha_after[:, 1])!r}, "
                f"alpha2_max={diagnostic_maximum(alpha_after[:, 1])!r}, "
                f"cation_min={diagnostic_minimum(after[:, CATION])!r}, "
                f"anion_min={diagnostic_minimum(after[:, ANION])!r}, "
                f"pva_min={diagnostic_minimum(after[:, PVA])!r}, "
                f"final_temperature_min_K="
                f"{diagnostic_minimum(final_temperature)!r}, "
                f"final_temperature_max_K="
                f"{diagnostic_maximum(final_temperature)!r}, "
                f"thermo_tmin_K={float(thermo.tmin)!r}, "
                f"thermo_tmax_K={float(thermo.tmax)!r}, "
                f"maximum_alpha_bound_excess="
                f"{diagnostic_maximum(alpha_bound_excess)!r}, "
                f"maximum_negative_inventory_violation="
                f"{diagnostic_maximum(negative_inventory_violation)!r}, "
                f"offending_flattened_cell_indices={offending_indices!r}"
            )
            # Classify only an otherwise numerically physical endpoint. Other
            # endpoint failures retain their existing numerical-error boundary.
            # The acceptance predicate and its roundoff allowance above are
            # unchanged; later conservation audits have not run at this point.
            if set(offending_indices) == {"temperature_above_tmax_tolerance"}:
                index = offending_indices["temperature_above_tmax_tolerance"]
                raise ConfiguredModelTemperatureRangeExceeded(
                    thermo.tmax, diagnostic_maximum(final_temperature), index,
                    temperature_output_roundoff[index],
                    "Local thermochemical endpoint diagnostics: "+endpoint_diagnostic,
                )
            raise PropagationCandidateNumericalError(
                "Nonphysical local thermochemical endpoint: "+endpoint_diagnostic
            )
