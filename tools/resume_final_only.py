...
# --- ECSP REUSE HANDOFF PATCH v3 ---
import sys as _rh_sys
from collections.abc import Mapping
from contextvars import ContextVar

# Capture methods before installing any wrapper, including inherited methods.
_rh_original_methods = {}
_rh_fresh_output = ContextVar("resume_final_fresh_baseline_handoff", default=None)
...
def _rh_load_persisted_handoff(item):
    ...
    metrics["reusedPersistedHandoff"] = True
    handoff["reusedPersistedHandoff"] = True
    result["metrics"] = metrics

    return result


def _rh_is_staggered_baseline(metadata, output_dir):
    """Authorize the external reference by its explicit role or exact path."""
    metadata = metadata if isinstance(metadata, Mapping) else {}
    if metadata.get("baseline_type") == "area_matched_staggered":
        return True
    if metadata.get("source_role") in {
        "post_optimization_area_matched_staggered_reference",
        "post_doe_area_matched_staggered_reference",
    }:
        return True
    output = Path(output_dir)
    return (
        output.name == "bc_handoff"
        and (
            output.parent.name == "area_matched_staggered_propagation"
            or (output.parent.name == "area_matched_staggered"
                and output.parent.parent.name == "propagation")
        )
    )


def _rh_original(self, method):
    for cls in type(self).__mro__:
        if cls in _rh_original_methods:
            original = _rh_original_methods[cls].get(method)
            if original is not None:
                return original
    raise RuntimeError(f"--reuse-handoff has no saved original {method}")


def _rh_resolve_handoff(self, item):
    _, _, metadata, output_dir = item
    output_dir = Path(output_dir)
    fields_path = output_dir / "bc_handoff_fields.npz"
    meta_path = output_dir / "bc_handoff_metadata.json"

    # Existing but invalid/incomplete artifacts are never permission to rerun.
    # Only an absent pair for the explicitly identified baseline can be fresh.
    if fields_path.exists() or meta_path.exists():
        return _rh_load_persisted_handoff(item)
    if not _rh_is_staggered_baseline(metadata, output_dir):
        return _rh_load_persisted_handoff(item)  # Candidate: fail closed.

    # evaluate_handoff normally delegates to self.evaluate_handoff_batch.
    # Preserve that original dispatch (including hybrid -> native delegation)
    # without changing any method during the call or entering the reuse wrapper
    # recursively. Authorization is scoped to this baseline output directory.
    original = _rh_original(self, "evaluate_handoff")
    token = _rh_fresh_output.set(output_dir.resolve())
    try:
        result = original(self, *item)
    finally:
        _rh_fresh_output.reset(token)
    result = dict(result)
    result["reusedPersistedHandoff"] = False
    for key in ("metrics", "handoff"):
        if isinstance(result.get(key), Mapping):
            result[key] = dict(result[key], reusedPersistedHandoff=False)
    if meta_path.is_file():
        saved = json.loads(meta_path.read_text(encoding="utf-8"))
        saved["reusedPersistedHandoff"] = False
...
def _rh_reuse_batch(self, items):
    items = list(items)
    fresh_output = _rh_fresh_output.get()
    if fresh_output is not None and all(
        Path(item[3]).resolve() == fresh_output for item in items
    ):
        return _rh_original(self, "evaluate_handoff_batch")(self, items)
    results = [_rh_resolve_handoff(self, item) for item in items]
    reused = sum(bool(result.get("reusedPersistedHandoff")) for result in results)
    print(
        "[resume-final] --reuse-handoff: "
        f"{reused}/{len(results)} persisted B/C handoffs reused; "
        f"{len(results) - reused} external baseline handoffs prepared",
        flush=True,
    )
    return results


def _rh_reuse_one(self, anode_mask, cathode_mask, metadata, output_dir):
    return _rh_resolve_handoff(
        self, (anode_mask, cathode_mask, metadata, output_dir)
    )


def _rh_install_patch(evaluator_classes=None):
    import importlib

    if evaluator_classes is None:
        evaluator_classes = []
        for module_name in ("ecsp_nsga2.evaluator", "ecsp_nsga2.bc_native"):
            module = importlib.import_module(module_name)
            evaluator_classes.extend(
                obj for obj in vars(module).values()
                if isinstance(obj, type)
                and callable(getattr(obj, "evaluate_handoff_batch", None))
            )
    classes = list(dict.fromkeys(evaluator_classes))
    if not classes:
        raise RuntimeError(
    ...
    # Two passes are essential: NativeBCGlobalEvaluator inherits the Python
    # evaluate_handoff method, which must be saved before its parent is patched.
    for cls in classes:
        if cls not in _rh_original_methods:
            _rh_original_methods[cls] = {
                name: getattr(cls, name, None)
                for name in ("evaluate_handoff", "evaluate_handoff_batch")
            }
    for cls in classes:
        cls.evaluate_handoff_batch = _rh_reuse_batch
        if _rh_original_methods[cls]["evaluate_handoff"] is not None:
            cls.evaluate_handoff = _rh_reuse_one

    print("[resume-final] --reuse-handoff enabled", flush=True)
    for cls in classes:
        print(f"[resume-final] patched {cls.__module__}.{cls.__name__}", flush=True)
