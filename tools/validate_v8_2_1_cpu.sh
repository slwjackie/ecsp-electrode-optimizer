...
OUT="${1:-$ROOT/runs/v8_2_1_cpu_validation}"
PYTHON_BIN="${PYTHON:-python}"
MODE="release"
if [[ "${ECSP_V821_PREPACKAGE:-0}" == "1" ]]; then
  MODE="prepackage"
fi

BASELINE_ZIP="${ECSP_V820_BASELINE_ZIP:-}"
if [[ -z "$BASELINE_ZIP" || ! -f "$BASELINE_ZIP" || -L "$BASELINE_ZIP" ]]; then
  echo "ECSP_V820_BASELINE_ZIP must name the immutable v8.2.0 ZIP regular file" >&2
  exit 2
fi
...
$PYTHON_BIN "$ROOT/python/audit_v820_preservation.py" "${AUDIT_ARGS[@]}"

if [[ "$MODE" == "release" ]]; then
  if [[ ! -f "$ROOT/PACKAGE_SHA256_MANIFEST_V8_2_1.txt" ]]; then
    echo "Missing authoritative PACKAGE_SHA256_MANIFEST_V8_2_1.txt" >&2
    exit 2
  fi
...
require(preservation.get("status") == "passed", "v8.2.0 preservation audit failed")
require(
...
payload = {
    "schema": "ecsp.v8.2.1-cpu-validation/v1",
    "status": "passed",
    "mode": mode,
    ...
    "results": {
        ...
        "v820_preservation": preservation["status"],
    ...
    "validated_package": {
        ...
        "expected_changes_sha256": hashlib.sha256(
            (package_root / "docs" / "V821_EXPECTED_CHANGES.json").read_bytes()
        ).hexdigest(),
        ...
        "authoritative_manifest_sha256": (
            hashlib.sha256(
                (package_root / "PACKAGE_SHA256_MANIFEST_V8_2_1.txt").read_bytes()
            ).hexdigest()
            if mode == "release"
            else None
        ),
